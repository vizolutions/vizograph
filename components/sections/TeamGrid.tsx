      ...
      <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {team.members.map((member) => (
          <li key={member.name} className={`${tile("glass", "p-3")} ${hoverLift}`}>
            <div className="relative aspect-square overflow-hidden rounded-3xl bg-card ring-1 ring-ink/8">
              <Image
                src={member.image}
                alt={member.name}
                fill
              ...
              />
              <div className="absolute inset-x-0 bottom-0 h-1/3 bg-linear-to-t from-panel/70 to-transparent" />
            </div>
